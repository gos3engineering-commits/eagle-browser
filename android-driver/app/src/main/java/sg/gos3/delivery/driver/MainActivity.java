...
public class MainActivity extends Activity {
    ...
    private static final String APP_PATCH =
        ...
        "var oldFetch=window.fetch;window.fetch=function(input,init){try{var u=String(input);if(u.indexOf('r=parcels')>=0&&init&&String(init.method||'GET').toUpperCase()==='POST'&&init.body){var b=JSON.parse(init.body);var w=window.__gos3ParcelWeights[String(b.sequence)];if(w){var sz=String(b.size||'Small').split('|KG:')[0];b.size=sz+'|KG:'+w;init=Object.assign({},init,{body:JSON.stringify(b)});}}}catch(e){}return oldFetch.call(this,input,init);};"+
        "function enhance(){"+
        ...
        " var save=document.getElementById('savep');if(save&&!document.getElementById('parcelWeight')){var step=document.querySelector('#modal .step');var m=step&&step.textContent.match(/Parcel\\s+(\\d+)/i);var n=m?m[1]:String(Object.keys(window.__gos3ParcelWeights).length+1);var box=document.createElement('div');box.style='margin:12px 0;padding:12px;border:1px solid #dbe1ea;border-radius:10px;background:#f8fafc';box.innerHTML='<label style="display:block;font-weight:700">Parcel Weight (kg)<input id="parcelWeight" type="number" step="0.001" min="0" inputmode="decimal" placeholder="Auto-detect from photo" style="width:100%;padding:11px;margin-top:5px;border:1px solid #ccd3dd;border-radius:9px"></label><div id="weightScanStatus" style="font-size:12px;color:#6b7280;margin-top:5px">Take a clear photo showing the KG label.</div>';save.parentNode.insertBefore(box,save);save.addEventListener('click',function(){var i=document.getElementById('parcelWeight');var v=i?String(i.value||'').trim():'';if(v){var num=parseFloat(v);if(isFinite(num)&&num>0)window.__gos3ParcelWeights[n]=num.toFixed(3);}else{window.__gos3ParcelWeights[n]='0.000';}},true);}"+
        " var submit=document.getElementById('submitStop');if(submit&&!document.getElementById('orderWeightTotal')){var list=document.getElementById('parlist');if(list){var cards=list.querySelectorAll('.parcel');cards.forEach(function(c,idx){var w=window.__gos3ParcelWeights[String(idx+1)];if(w&&!c.querySelector('.gos3kg')){var d=document.createElement('div');d.className='muted gos3kg';d.innerHTML='Weight: <b>'+w+' kg</b>';c.appendChild(d);}});}var total=0;Object.keys(window.__gos3ParcelWeights).forEach(function(k){total+=parseFloat(window.__gos3ParcelWeights[k]||0)||0;});var t=document.createElement('div');t.id='orderWeightTotal';t.style='margin:12px 0;padding:12px;border-radius:10px;background:#ecfdf5;font-size:18px;font-weight:800';t.textContent='TOTAL ORDER WEIGHT: '+total.toFixed(3)+' kg';submit.parentNode.insertBefore(t,submit);}"+
        " var success=[].slice.call(document.querySelectorAll('.card h2')).find(function(x){return /DELIVERY SUBMITTED/i.test(x.textContent||'');});if(success&&!document.getElementById('submittedWeightTotal')){var total2=0;Object.keys(window.__gos3ParcelWeights).forEach(function(k){total2+=parseFloat(window.__gos3ParcelWeights[k]||0)||0;});var p=document.createElement('p');p.id='submittedWeightTotal';p.innerHTML='<b>Total Order Weight: '+total2.toFixed(3)+' kg</b>';success.parentNode.insertBefore(p,success.nextSibling);}"+
        "}"+
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        s.setUserAgentString(s.getUserAgentString() + " GOS3DriverAndroid/1.6");
